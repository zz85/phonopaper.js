...
function Spectrogram(filename, options) {
	...
	gf.init(frequencyToDetect, this.sampleRate, chunkSize);
	this.gf = gf;
...
Spectrogram.prototype.process = function(e) {
	...
	for (var i = 0; i < this.freqs.length; i++) {
		...
		var l = 1 - k;
		this.buffer[i] = this.convert(this.freqs[i]) * (1 - (l * l)) * 10;
	}
	...
	for (var i = 0; i < this.freqs.length; i++) {
		...
		// this.buffer2[i] = this.buffer2[i] > 0.75 ? this.buffer2[i] : this.buffer2[i] * 0.5;
